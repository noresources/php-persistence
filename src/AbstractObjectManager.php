 ...
 */
abstract class AbstractObjectManager implements ObjectManager
{
	...
	public function refresh($object)
	{
		...
		if (isset($this->unitOfWork))
		{
			...
			{
				$objectId = $this->unitOfWork->getObjectIdentity(
					$object);
			}
		}

		if ($objectId == NULL && $this->hasRepository($className) &&
			($repository = $this->getRepository($className)) &&
			($repository instanceof ObjectContainerInterface) &&
			$repository->contains($object))
		{
			$objectId = $repository->getObjectIdentity($object);
		}

		if ($objectId === NULL)
		{
			// Not reliable
			//$objectId = $class->getIdentifierValues($object);

			throw new \RuntimeException('Could not get object ID');
		}
	...
	{
		return \is_object($this->unitOfWork);
	}
